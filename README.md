...
`va_list` is an **object type** suitable for holding the information needed by the macros `va_start`, `va_copy`, `va_arg`, and `va_end`. In other words, it is a list that will contain all the dynamic arguments.
...
It doesn't hold the values itself—it's more like a pointer that knows where the first "extra" arguments start in memory.
...
2. **fp_offset**: Tracks position in **floating-point registers**. It's an offset in bytes into a separate area of `reg_save_area`. Increases by 16 each time you call `va_arg()` for a float/double.
...
An offset is basically **how far (in memory bytes)** from the start of the arguments you are currently pointing.
...
## Argument Passing on x86-64 (System V ABI)
...
## What Happens with Each printf Specifier
...
**But here's the problem:** registers are temporary! The function needs to use these registers for its own work.
